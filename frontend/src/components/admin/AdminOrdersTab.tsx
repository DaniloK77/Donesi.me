          ...
          >
            ...
            <dl className="mt-4 grid gap-3 sm:grid-cols-3">
              ...
              <div>
                ...
                <dd className="truncate text-[12px] text-brand-ink/50">
                  {copy.paymentLabel} · {order.items.length} ×{" "}
                  {copy.itemsLabel.toLowerCase()}
                </dd>
                {order.estimate ? (
                  <dd className="mt-0.5 text-[12px] text-brand-ink/50">
                    {copy.estimateLabel}: {order.estimate.minutes}{" "}
                    {copy.minutesShort}
                  </dd>
                ) : null}
                {order.cancellation.cancelledBy ? (
                  <dd className="mt-0.5 text-[12px] text-red-700">
                    {copy.cancelledByLabel[order.cancellation.cancelledBy]}
                  </dd>
                ) : null}
              </div>
            </dl>
