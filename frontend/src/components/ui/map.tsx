...
/*
 * mapcn intentionally bridges React state with MapLibre's imperative refs.
 * The registry implementation updates callback refs during render so native
 * map listeners always call the latest prop, and queues a style transition
 * from an effect. These patterns are isolated to this owned vendor component.
 */
...
type MapMarkerProps = {
  ...
  children: ReactNode;
  /** Accessible label for the interactive marker element */
  ariaLabel?: string;
...
function MapMarker({
  ...
  useEffect(() => {
    marker
      .getElement()
      .setAttribute("aria-label", ariaLabel ?? "Map marker");
  }, [ariaLabel, marker]);
