...
function DeliveryTrackingMapSection({
  ...
  /** The delivery promise made at confirmation, when there is one. */
  ...
  // Prefer the promise the customer was actually given at confirmation. The
  // simulation's own ETA is only a fallback for orders placed before estimates
  // existed, so the card and the map never quote two different numbers.
  const promisedMinutesLeft = toMinutes(useCountdown(promisedArrivalAt) ?? 0);
  const etaMinutes = promisedArrivalAt
    ? promisedMinutesLeft
        ...
        >
          ...
          {!hasArrived ? (
            <strong className="text-[14px] font-bold text-brand-ink">
              {etaMinutes} {content.minutesShort}
            </strong>
          ) : null}
        </div>
      </div>
...
export default function OrderDeliveryTracking({
  ...
  return (
    <DeliveryTrackingMapSection
      ...
      promisedArrivalAt={order.estimate?.at ?? null}
